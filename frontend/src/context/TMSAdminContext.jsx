...
export const TMSAdminProvider = ({ children }) => {
  ...
  // Helper functions
  const T = () => window.TMS || { branches: [], supervisors: [], vehicles: [], drivers: [], clients: [], customers: [], locations: [], routes: [], bunks: [], trips: [], exceptions: [], gpsLog: [], users: [], permissions: [], distanceChecks: [], radiusAlerts: [], supervisorNotices: [], B: {}, S: {}, V: {}, D: {}, C: {}, U: {}, L: {}, R: {}, F: {}, T: {} };

  const fmtPhone = (d) => { d = String(d || ''); return d.length === 10 ? d.slice(0, 5) + ' ' + d.slice(5) : d; };
  const fmtImei = (d) => { d = String(d || ''); return d.length === 15 ? `${d.slice(0, 2)} ${d.slice(2, 8)} ${d.slice(8, 14)} ${d.slice(14)}` : d; };
  const stampNow = () => {
