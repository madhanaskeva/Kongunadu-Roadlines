...
export const Dashboard = () => {
  ...
  const dashCards = (currentCfg.cards || []).map(it => ({ ...cat.cards[it.src], ...it, label: it.title || (cat.cards[it.src] || {}).label })).filter(Boolean);
  const dashCharts = (currentCfg.charts || []).map(it => ({ ...cat.charts[it.src], ...it, title: it.title || (cat.charts[it.src] || {}).title })).filter(Boolean);
  const dashLists = (currentCfg.lists || []).map(it => ({ ...cat.lists[it.src], ...it, title: it.title || (cat.lists[it.src] || {}).title })).filter(Boolean);
